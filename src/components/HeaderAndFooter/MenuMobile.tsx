...
const MenuMobile = ({
  ...
  const handleCatMenuToggle = () => {
    setShowCatMenu((prev) => {
      if (!prev) setShowContactMenu(false); // Close Contact Menu if opening Cat Menu
      return !prev;
    });
  };

  const handleContactMenuToggle = () => {
    setShowContactMenu((prev) => {
      if (!prev) setShowCatMenu(false); // Close Cat Menu if opening Contact Menu
      return !prev;
    });
  };
