...
<Heading className=' text-left' title="Benefits of BML Insurance" />
    <p className=' text-lg md:text-2xl xl:text-4xl flex items-center max-lg:-mt-6 justify-center h-full lg:-mt-10 mb-4  '>What makes BML Insurance for Business one of India's most favorite place to buy Marine Insurance?</p>
        </div>
    <div className='flex flex-wrap gap-4 lg:min-w-[420px] max-lg:mt-16'>
    {data.map((item,index) => (
        <div key={item.id} className={` p-4  justify-center items-center shadow-lg w-[200px] border text-left  rounded-lg ${index % 2 === 0 && "lg:-translate-y-10" } `}>
          <Image width={400} height={400}  src={item.icon} alt={item.name} className=" w-16 !object-cover  mb-2 md:mb-6 " />
                <div className=''>
          <h3 className="text-gray-600 text-lg  uppercase font-[500] mb-2">{item.name}</h3>
  ...
  <div className='pt-10'>
  <div className="absolute inset-0 bg-amber-50/60"></div> {/* Overlay */}
    <Image 
