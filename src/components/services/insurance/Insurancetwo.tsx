...
const Insurancetwo = () => {
  return (
    <div className='sjcontainer px-4 my-12 md:my-24'>
      <div className='mb-4'>
    <Heading title="Why Choose BML Cargo Insurance?" className='text-left' />
      <p className='text-gray-600 mt-1 text-base md:text-lg'>Experience all the benefits of our global coverage.</p>
      </div>
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-2">
      {data.map((item) => (
        <div key={item.id} className=" py-4 md:p-4 text-left  rounded-lg ">
          <img src={item.icon} alt={item.name} className=" w-40 md:w-60  mb-2 md:mb-6 -ml-5 md:-ml-12" />
          <h3 className=" text-xl font-[600] mb-2">{item.name}</h3>
          <p className="text-gray-600  text-base md:text-[15px]">{item.content}</p>
        </div>
      ))}
    </div>
      </div>
