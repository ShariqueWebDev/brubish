...
const Financefour = () => {
  return (
    <div className='sjcontainer my-8 md:my-16'>
         <Heading title="Why BML?" className=' mb-2 md:mb-4' />
         <div className=' p-3 md:p-6 bg-gradient-to-tr from-red-400/50 via-red-200 to-amber-300 text-balance '>
            <p className=' text-lg md:text-xl font-[500]'>BML provides new-age customized financing 
            products for your business needs</p>
            <div className='flex flex-wrap gap-x-2 mt-4'>
        {
            financeBenefits.map((benefit) => (
              <div key={benefit.title} className='flex items-center mb-4 bg-white hover:bg-gray-100 border p-2.5 px-3.5 shadow-[0_3px_10px_rgb(0,0,0,0.2)] rounded-md mx-2'>
                <img src={benefit.icon} alt={benefit.title} className='w-6 h-6 mr-4' />
                <p>{benefit.title}</p>
              </div>
            ))
        }
