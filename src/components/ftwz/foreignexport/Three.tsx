...
export default function Threeforeignexport() {
  const comparisons = [
    ...
    },
    {
      ...
      customBonded: "Requires customs assessment at the gateway, leading to longer turnaround times.",
    },
    {
    ...
    },
    {
      ...
      customBonded: "Consolidation from multiple countries/suppliers is not permitted.",
    },
    {
      ...
      customBonded: "Storage beyond one year requires customs approval; interest on duty is levied after 90 days.",
    },
    {
    ...
    },
    {
      ...
      ftwz: "Done within BML FTWZ; duty paid for moved quantity; flexible BL/invoice combinations.",
      customBonded: "Done at customs house, away from the warehouse.",
    },
    {
      icon: <FileCheck className="w-6 h-6" strokeWidth={1}/>,
      parameter: "Customs Clearance for Export/Re-export",
      ftwz: "Single-window, seamless process within BML FTWZ.",
  ...
  return (
    <div className=" py-10 sjcontainer text-gray-600  max-md:overflow-x-scroll">
      <Subheading className=" mb-6 text-center" title="Why Choose BML FTWZ Foreign Exporters over other Customs Bonded Warehouses?"  />
      <table className="w-full border-collapse border border-gray-300">
        <thead>
          <tr className="bg-gray-200">
            <th className="min-w-[275px] border border-gray-300 px-4 py-2 text-lg font-medium text-left">Particulars</th>
            <th className="border border-gray-300 px-4 py-2 text-left text-lg font-medium">FTWZ</th>
            <th className="border border-gray-300 px-4 py-2 text-left text-lg font-medium">Custom Bonded Warehouse</th>
          </tr>
        </thead>
        <tbody>
          {comparisons.map((item, index) => (
            <tr key={index} className={`group text-sm ${index % 2 === 0 ? "bg-gray-50" : "bg-white"}`}>
